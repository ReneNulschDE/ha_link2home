...
class Link2HomeWebApi:
    ...
    def _create_sign(self, data):
        ...
        signature = base64.b64encode(
            private_key.sign(data_bytes, padding.PKCS1v15(), hashes.SHA1())
        )
    ...
    async def login(self) -> bool:
        ...
        login_response = await self._request(
            "post", "/api/service/user/login", data=data
        )
    ...
    async def get_device_list(self) -> dict[str, Link2HomeDevice]:
        ...
        response = await self._request("get", f"/api/app/device/list", params=data)
        ...
        if response.get("code") and response.get("code") == 600:
            ...
            return {result.macAddress: result for result in devices}

        LOGGER.warn("get_device_dist failed: %s", response)
        return {}
