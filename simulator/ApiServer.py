...
class Link2HomeSimulatorServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/service/user/login":
            with open("./api/service/user/login", "rb") as file:
                self.wfile.write(file.read())
            return
        if parsed.path == "/api/app/device/list":
            if os.path.isfile("./api/app/device/.list-dev"):
                with open("./api/app/device/.list-dev", "rb") as file:
                    self.wfile.write(file.read())  # Read the file and send the contents
            else:
                with open("./api/app/device/list", "rb") as file:
                    self.wfile.write(file.read())  # Read the file and send the contents
            return
